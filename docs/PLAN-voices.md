...
|---|---|
| `YouTubeClient` | `search()`, `videos()`, `comments()`. REST over the existing curl transport pattern (`fn(array $requests): array`) so tests inject fixtures. **`order=relevance`, never `order=viewCount`** (§9). HTML-unescape every title. Two pages of each of two queries, filter titles on *locust* + *manor*, resolve statistics and full snippets with `videos.list`, apply the Roblox exclusion, sort by views locally, take ten. Optionally (`voices.archive`, `ArchiveSource`) the most watched ids of an archive built by a date-ordered crawl join the candidates first: relevance is a sample, and it once left out a video with thirty times the views of the #15. |
...
| Transcript request blocked | One refused video is only a suspicion, since YouTube walls single videos too: the next video is asked after ten minutes instead of two, and a video refused while the address is served is cached as `missing` and reported. Three refusals in a row on different videos stop every further caption request of the run and writes `.blocked.json` in the transcript cache, so no run asks anything until the pause is over. The pause grows with refusals in a row — 6 h, 12 h, then 24 h (`voices.blockCooldownHours`) — and the count resets on the first answer that gets through; `bin/voices --clear-block` ends it by hand. On the workstation each refusal raises a critical desktop notification and schedules a one-shot systemd timer that reruns the unit when the pause ends. Caption errors that are not refusals are retried as before and only reported, with one desktop alert per run. Caption requests are spaced two minutes apart (`voices.transcriptInterval`). Summaries fall back to the cache and to comments, the run still publishes, and the log says so. |
